...
for api in ${haproxy_api_labels}; do
  tmpusers=$(curl -X GET \
    ...
    -H "Use-API: ${api}" \
    ${lb}/hub/api/users 2> /dev/null | python -m json.tool | grep server | grep -v null | cut -d\" -f 4 | cut -d/ -f 3)
  for user in ${tmpusers}; do
    users="${users} ${user}"
  done
done

now=$(date "+%Y-%m-%d %H:%M:%S")
usercount=$(echo "${users}" | wc -w)
echo "${now}|${usercount}|$(echo ${users})"
